...
public class move : MonoBehaviour
{
       ...
       if(Mathf.Abs(Dir.y)>0f || Sprinting||dashPressed)
        {
            ...
            cam.fieldOfView = Mathf.Lerp(cam.fieldOfView, MaxFov,5.5f*Time.deltaTime);
        }else if(Dir.y ==0f||Dir.magnitude ==0f )
        {
            FovMultiplier += 0.5f;
            cam.fieldOfView = Mathf.Lerp(cam.fieldOfView, defaulrFOv, 5.5f * Time.deltaTime);
    ...
    void DoWallRunMovemntLeft()
    {
       // Debug.Log(y);
        rb.drag = 0f;
        if (Input.GetKeyDown(KeyCode.D) || Input.GetKeyDown(KeyCode.A)) rb.velocity = new Vector3(rb.velocity.x, 0f, rb.velocity.z);
        ...
        {
            rb.AddForce(transform.up * Time.deltaTime * wallJumpforce*0.8f, ForceMode.Impulse);
